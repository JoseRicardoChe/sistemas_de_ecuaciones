...
public class csMetodoOne {
    ...
    public ArrayList<csFilaOne> One(double y, double x1, double x2, int it) {
        ...
        for (int i = 0; i < (copyIt + 1); i++) {
            ...
            fila.setFx(copyY + h * this.Equation(fila.getX())); //Asignar el resultado de copyY + h por la ecuacion llamado equation
            copyY = fila.getFx();
            lista.add(fila);
        }
        Calcular v = new Calcular();
        v.main();
        return lista;
    }
